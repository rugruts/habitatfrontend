...
-- Complete email templates and automations for Habitat Lobby
-- ===================================================================
...
-- 4. SEPA Payment Instructions
(
  'booking_confirmation_sepa_instructions',
  'Payment Instructions - {{property_name}} | Habitat Lobby',
  '<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p>Complete your booking with SEPA bank transfer</p>
    </div>
    
    <div class="content">
      <p>Dear {{customer_name}},</p>
      
      <p>Thank you for your booking! Please complete your payment using the SEPA bank transfer details below.</p>
      ...
      <div class="payment-info">
        ...
        <div class="detail-row">
          ...
          <span class="code">{{sepa_iban}}</span>
        </div>
        <div class="detail-row">
          <span><strong>BIC/SWIFT:</strong></span>
          <span class="code">{{sepa_bic}}</span>
        </div>
        <div class="detail-row">
          <span><strong>Account Holder:</strong></span>
          <span>{{sepa_account_holder}}</span>
        </div>
        <div class="detail-row">
          <span><strong>Reference Code:</strong></span>
          <span class="code">{{sepa_reference}}</span>
        </div>
        <div class="detail-row">
          <span><strong>Payment Deadline:</strong></span>
          <span><strong>{{payment_deadline}}</strong></span>
        </div>
      </div>
      ...
      <div class="warning">
        ...
        <ol style="margin-bottom: 0;">
          <li><strong>Include the reference code</strong> in your transfer description</li>
          <li>Complete the transfer before <strong>{{payment_deadline}}</strong></li>
    ...
    <div class="footer">
      <p><strong>Habitat Lobby</strong> | Trikala, Greece</p>
      <p>Booking: {{booking_id}} | Reference: {{sepa_reference}}</p>
      <p>Support: <a href="mailto:{{business_email}}">{{business_email}}</a> | {{business_phone}}</p>
    </div>
  </div>
</body>
</html>',
  ...
  true,
  ARRAY['customer_name', 'total_amount', 'currency_symbol', 'sepa_iban', 'sepa_bic', 'sepa_account_holder', 'sepa_reference', 'payment_deadline', 'booking_id', 'business_email', 'business_phone'],
  NOW(),
  NOW()
);
...
-- Create custom_map_pins table if it doesn't exist
...
-- Insert sample custom map pins for Trikala attractions
-- Note: This will skip duplicates if they already exist
INSERT INTO custom_map_pins (
  name,
  description,
  latitude,
  longitude,
...
('Trikala Central Square', 'Historic central square with cafes and shops', 39.5555, 21.7686, 'map-pin', '#2D5A27', 'attraction', true, NOW(), NOW()),
('Meteora Monasteries', 'UNESCO World Heritage site with stunning monasteries', 39.7153, 21.6302, 'mountain', '#8B4513', 'attraction', true, NOW(), NOW()),
('Litheos River', 'Beautiful river running through Trikala', 39.5575, 21.7725, 'waves', '#1E90FF', 'nature', true, NOW(), NOW()),
('Trikala Castle', 'Byzantine fortress overlooking the city', 39.5580, 21.7720, 'castle', '#8B4513', 'attraction', true, NOW(), NOW()),
('Mill of Elves Park', 'Charming park with fairy tale theme', 39.5590, 21.7710, 'trees', '#228B22', 'nature', true, NOW(), NOW()),
('Trikala Railway Station', 'Historic train station', 39.5540, 21.7650, 'train', '#FF6347', 'transport', true, NOW(), NOW())
ON CONFLICT DO NOTHING;
...
-- Check custom map pins
SELECT name, category, is_active FROM custom_map_pins ORDER BY created_at DESC;
...
1. EMAIL TEMPLATES:
   - Stripe booking confirmation (immediate)
   - Pre-arrival instructions (48h before check-in)
   - Post-stay review request (24h after check-out)
   - SEPA payment instructions

2. EMAIL AUTOMATIONS:
   - Automatic booking confirmations
   - Pre-arrival instruction delivery
   - Post-stay review requests

3. SYSTEM SETTINGS:
   - Configurable email and booking parameters
   - Production-ready defaults

4. SAMPLE DATA:
   - Custom map pins for Trikala attractions

To activate in production:
1. Run this SQL script
2. Set up cron job for scheduled email processing
3. Configure SMTP settings in backend/.env
4. Test automation triggers
5. Monitor email logs for delivery status
*/